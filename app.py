...
# ==========================================
st.set_page_config(page_title="BIST Tüm Tavan & Algo Terminal", layout="wide")
...
# ==========================================
# 2. BIST TÜM HİSSE LİSTESİ (OTOMATİK YÜKLEME)
# ==========================================
BIST_TUM_LIST = [
...
st.title("⚡ BIST Tüm Tavan Hunter & Algo-Trading Terminal")

# SEKMELİ YAPI (TABS)
tab1, tab2 = st.tabs(["🚀 BIST Tüm Tavan & İvme Taraması", "📊 Scalp & Detaylı Analiz"])
...
with tab1:
    ...
    with col_scan1:
        st.info(f"Sistemde kayıtlı **{len(BIST_TUM_LIST)} adet BIST hissesi** otomatik olarak taranacaktır.")
    with col_scan2:
        start_scan = st.button("🚀 BIST TÜM'ü Tara", use_container_width=True)
    ...
    if start_scan:
        ...
        for i, t in enumerate(BIST_TUM_LIST):
            status_text.text(f"Taranıyor ({i+1}/{len(BIST_TUM_LIST)}): {t}")
        ...
        # Öncelikli Tavan Adaylarını En Üste Getir
        df_results = df_results.sort_values(by="Sinyal", ascending=False)
...
# ==========================================
with tab2:
    symbol = st.text_input("Detaylı Analiz Edilecek Hisse (Örn: THYAO.IS, DOFRB.IS)", value="THYAO.IS")
