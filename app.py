...
st.caption(
    "RSI, MACD, Dinamik ATR Stop-Loss, Hacim Patlaması & Tavan Avcısı Modülü"
)
...
analiz_modu = st.sidebar.radio(
    "🎯 Analiz Modu Seçin:",
    ["📈 Genel Trend & Al-Sat Modu", "🚀 Tavan Avcısı (Sıkışma & Hacim)"],
    help=(
        "Tavan Avcısı modu, ertesi gün %8.5+ yükseliş potansiyeli gösteren"
        " hacim patlaması ve sıkışma yaşayan hisselere odaklanır."
    ),
)
...
if st.sidebar.button("🚀 Taramayı Başlat"):
    ...
    if not hisseler:
        ...
        for idx, hisse in enumerate(hisseler):
            try:
                ...
                if len(df) > 60:
                    ...
                    # Tavan Avcısı Özel İndikatörleri (Hacim Patlaması & Sıkışma)
                    ...
                    # MOD SEÇİMİNE GÖRE MODEL HEDEFİ (TARGET)
                    if "Tavan Avcısı" in analiz_modu:
                        # Gelecek en yüksek fiyat %8.5+ üzerine çıktı mı? (Tavan İhtimali)
                        df["Target"] = (
                            df["High"].shift(-vade_gun)
                            >= df["Close"] * 1.085
                        ).astype(int)
                        features = [
                            "Returns",
                    ...
                    if len(X) > 30 and y.sum() > 0:
                        # Dengesiz veriyi dengelemek için class_weight='balanced'
                        ...
                        son_hacim_kat = float(df_model["Vol_Ratio"].iloc[-1])
                        ...
                        sonuclar.append({
                            ...
                            (
                                "Tavan İhtimali (%)"
                                if "Tavan Avcısı" in analiz_modu
                                else "Yükseliş İhtimali (%)"
                            ): round(olasilik * 100, 1),
                            "Hacim Katı (x Ort)": round(son_hacim_kat, 2),
        ...
        if sonuclar:
            ...
            st.session_state["prob_col"] = prob_col
        else:
            st.warning(
                "Veri çekilemedi veya tavan şartlarına uyan hisse bulunamadı."
            )
...
if "res_df" in st.session_state and not st.session_state["res_df"].empty:
    ...
    prob_col = st.session_state.get("prob_col", "Yükseliş İhtimali (%)")
    tp_col_name = f"Hedef Kar-Al (1:{rr_val})"

    st.success("Analiz Tamamlandı!")

    top_count = min(5, len(res_df))
    st.subheader(f"🔥 Öne Çıkan İlk {top_count} Hisse")
    ...
    for i, (_, row) in enumerate(top_n.iterrows()):
        with cols[i]:
            st.metric(
                ...
                delta=f"{prob_col}: %{row[prob_col]}",
                delta_color="normal",
            )
            st.caption(f"💥 Hacim: **{row['Hacim Katı (x Ort)']}x**")
