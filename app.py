...
def fetch_market_data(symbol, period="5d", interval="15m"):
    try:
        ...
        # RVOL (Hacim Patlaması Katı)
        df['RVOL'] = df['Volume'] / (df['Volume'].rolling(20).mean() + 1e-9)
        
        # Momentum (3 Periyotluk Değişim)
...
# ==========================================
# 4. GELİŞMİŞ GÜVENİLİR TAVAN ANALİZ MOTORU
# ==========================================
def analyze_advanced_tavan_potential(df):
    """
    Kombine Güvenilir Tavan Analiz Mantığı:
    1. Kural Tabanlı Teknik Filtreler
    2. ML (RandomForest) Olasılık Hesabı
    3. Güven Skoru Doğrulaması
    """
    ...
    # TEKNİK FİLTRE KURALLARI (Güvenilirlik Puanlaması)
    rule_score = 0
    reasons = []

    # Kural 1: VWAP ve Trend Onayı
    if last_c > vwap:
        rule_score += 25
        reasons.append("Fiyat VWAP üzerinde (Alıcılar baskın)")
    if ema20 > ema50:
        rule_score += 15
        reasons.append("Kısa vadeli trend pozitif (EMA20 > EMA50)")

    # Kural 2: Hacim Patlaması
    if rvol >= 2.0:
        rule_score += 25
        reasons.append(f"Olağanüstü Hacim ({round(rvol, 1)}x Kat)")
    elif rvol >= 1.3:
        rule_score += 15
        reasons.append("Hacim ortalamanın üzerinde")

    # Kural 3: Zirveye Yakınlık ve İvme
    if last_c >= day_high * 0.985:
        rule_score += 20
        reasons.append("Günün en yüksek seviyesine yakın")

    # Kural 4: Tavana Uygun Mesafe (Çok yakınsa marj kalmamıştır, çok uzaksa imkansızdır)
    if 1.5 <= tavan_mesafe_pct <= 7.0:
        rule_score += 15
        reasons.append(f"Tavana ideal yaklaşma marjı (%{round(tavan_mesafe_pct, 1)})")

    # Toplam Karma Skoru Hesapla (Kural Skoru %60 + AI Skoru %40)
    composite_score = round((rule_score * 0.6) + (ai_prob * 0.4), 1)

    # Durum Belirleme ve Risk Tuzak Filtresi
    ...
    return {
        ...
        "Durum": status,
        "Nedeni": " | ".join(reasons) if reasons else "Yetersiz teknik formasyon."
    }
...
# ==========================================
# MODÜL 1: BIST GÜVENİLİR TAVAN MOTORU
# ==========================================
if selected_market == "🇹🇷 BIST Güvenilir Tavan Motoru":
    ...
    with tab1:
        ...
        st.caption("Filtreler: VWAP Üstü Tutunma, Zirve Kırılımı, Hacim Patlaması (RVOL) ve Yapay Zeka Onayı.")

        if st.button("🚀 Güvenilir BIST Taramasını Başlat"):
            results = []
            telegram_alerts = []
            bar = st.progress(0)
            ...
            if results:
                ...
                if telegram_alerts:
                    ...
                    st.info("ℹ️ Şu anki piyasa koşullarında yüksek tavan onay kriterini karşılayan hisse bulunamadı.")
...
# ==========================================
    ...
    st.info("RSI ve VWAP kombinasyon stratejisinin geçmiş performans simülasyonu.")
